import { cached, normalizeCacheKey } from "./cache.js";

const RAPIDAPI_HOST = "uk-golf-course-data-api.p.rapidapi.com";
const BASE_URL = `https://${RAPIDAPI_HOST}`;
const RAPIDAPI_TTL_SECONDS = 30 * 24 * 60 * 60;
...
function consumeBudget(budget) {
  ...
  if (budget.remaining <= 0) {
    throw new RapidAPIError("RapidAPI request budget reached.", 429, "request_budget_reached");
  }
  budget.remaining -= 1;
}
...
async function request(path, query = {}, options = {}) {
  ...
  const cacheKey = normalizeCacheKey(["rapidapi", path, url.searchParams.toString()]);
  const { payload } = await cached(cacheKey, RAPIDAPI_TTL_SECONDS, async () => {
    consumeBudget(options.budget);
...
export async function searchClubs(query, options = {}) {
  const payload = await request("/clubs", { search: query }, options);
  return unwrapArray(payload, ["data", "clubs", "results"]).map(normalizeClub);
}

export async function fetchClubCourses(clubId, options = {}) {
  const payload = await request(`/clubs/${encodeURIComponent(clubId)}`, {}, options);
  return unwrapArray(payload, ["data", "courses", "results"]).map(normalizeCourseSummary);
}

export async function fetchScorecard(courseId, options = {}) {
  const payload = await request(`/courses/${encodeURIComponent(courseId)}`, {}, options);
...
export async function searchCourses(query, options = {}) {
  ...
  const clubs = await searchClubs(query, { budget });
  const results = [];
  const seen = new Set();
...
export async function searchMultipleQueries(queries, options = {}) {
  ...
  const stopAfterFirstQueryWithResults = options.stopAfterFirstQueryWithResults ?? false;

  for (const query of queries) {
    let matches = [];
    try {
      matches = await searchCourses(query, options);
    } catch (error) {
      if (error instanceof RapidAPIError && error.code === "request_budget_reached") {
        return results;
      }
    ...
    if (stopAfterFirstQueryWithResults && results.length > 0) {
      return results;
    }
  }
